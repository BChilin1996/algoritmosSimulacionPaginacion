...
@Component({
  selector: 'app-reemplazo-segunda-oportunidad',
  standalone: true,
  imports: [ReactiveFormsModule, NgFor, NgIf],
  templateUrl: './reemplazo-segunda-oportunidad.component.html',
  styleUrl: './reemplazo-segunda-oportunidad.component.css'
})
export class ReemplazoSegundaOportunidadComponent {
  ...
  terminarProceso(idProcesoEliminar: Number) {
    ...
    for (let i = 0; i < marcoMPAEliminar.length; i++) {
      ...
      this.marcosMemoriaPrincipal[indice].nombreProceso = null;
      this.marcosMemoriaPrincipal[indice].numeroPagina = null;
      this.marcosMemoriaPrincipal[indice].bitReferencia = null;
    }
  ...
  cargarAMemoriaPrincipal(idProcesoReemplazar: Number, nombreProcesoReemplazar: String) {
    ...
    if (falloPagina) {
      ...
      for (let i = 0; i < reiniciarPaginaReferenciada.length; i++) {
        ...
        this.marcosMemoriaPrincipal[indice].bitReferencia = 1;
      }
      ...
      let marcosBitReferencia0 = this.marcosMemoriaPrincipal.find((option: any) => option.bitReferencia === 0);

      if (!marcosBitReferencia0){
        for (let i = 0; i <  this.marcosMemoriaPrincipal.length; i++) {
          let indice = Number( this.marcosMemoriaPrincipal[i].numeroMarco) - 1;
          this.marcosMemoriaPrincipal[indice].bitReferencia = 0;
        }
      }

      marcosBitReferencia0 = this.marcosMemoriaPrincipal.find((option: any) => option.bitReferencia === 0);

      const procesoMarcoLiberar = this.marcosMemoriaPrincipal.find((option: any) => option.idProceso === marcosBitReferencia0.idProceso);
  ...
  agregarProcesoSegundaOportunidad() {
      ...
      if (this.memoriaPrincipalLlena === false) {
        ...
        for (let i = 0; i < marcosMemoriaPrincipalVacia.length; i++) {
          ...
          if (cantidadMarcosAsignados < numeroPaginas) {
            ...
            this.marcosMemoriaPrincipal[indices].bitReferencia = 0;
